...
| `debate.md` | Reconcile independently-derived work with an Architect from another provider. Determine your model first and stop if it is below Architect - read `_strata/universal/harness-codex.md` if you are Codex or `_strata/universal/harness-claude-code.md` if you are Claude Code for how, before the file, the brief, or anything else |
