...
- **Delegation hard gate — mandatory:**
  - Classify every delegated task by tier before choosing the model.
  ...
  - Proactive or parallel delegation means multiple in-session `spawn_agent` calls. Respect the advertised concurrency limit; with four total slots including the root, run a six-seat DAP council in two batches of three.
