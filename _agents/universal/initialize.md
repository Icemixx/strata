...
supervisor surface, `AGENT_SYSTEM_CHECK`, worker-chain budgets, certification states, and retrofit
...
5. **`_agents/.kit-version`** — one line holding the canonical commit SHA this repo's kit copy was taken from. Project-owned and deliberately **outside** `_agents/universal/`, because that folder is copied wholesale and a marker inside it would be overwritten on every sync. It is what makes the session-start freshness check possible; without it the check has nothing to compare against and reports nothing.
...
- Source and empirical certification are reported separately; initialization is not certified complete while empirical verification is pending.
